...
cycle:
  ...
  switch(x) {
    case '*': Serout(0); Serout('\r'); Wait(500);  // clear the line
              Printx("T,Sounder-2\r",0); Wait(1000);
              entries=SendDirectory();
              for(i=entries;i<=10;i++)
                Serout(',');
              Printx("AUX1,AUX2\r",0);
              Wait(2000);
    ...
    case 'C':
    case 'D':
    case 'E': PlayOut(index*0x10+0x10); break;
    case 'K': bitSet(PORTA,OUTX1); break;
    case 'L': bitSet(PORTA,OUTX2); break;
    case '@': bitClear(PORTA,OUTX1);
              bitClear(PORTA,OUTX2);
              break;
